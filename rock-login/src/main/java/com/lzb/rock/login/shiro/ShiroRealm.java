...
import com.lzb.rock.base.busInterface.IShiro;
...
public class ShiroRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) throws AuthenticationException {
		...
		String userPassword = shiroUser.getUserPassword();// 用户密码
		String salt = shiroUser.getUserSalt();// 加密随机字符串
		String userAccount = shiroUser.getUserAccount();// 账号
		ByteSource credentialsSalt = new Md5Hash(salt);
		SimpleAuthenticationInfo info = new SimpleAuthenticationInfo(shiroUser, userPassword, credentialsSalt, userAccount);
	...
	private void getMenuChild(Set<ShiroAuthz> authzs, List<ShiroMenu> menusSort, String menuCode) {
		...
		for (ShiroAuthz authz : authzs) {
			...
			if (menuCode.equals(authz.getAuthzPcode())) {
				...
				shiroMenu.setMenuPCode(authz.getAuthzPcode());
	...
	public void setCredentialsMatcher(CredentialsMatcher credentialsMatcher) {
		...
		md5CredentialsMatcher.setHashAlgorithmName(ShiroConfig.hashAlgorithmName);// 指定加密算法
		md5CredentialsMatcher.setHashIterations(ShiroConfig.hashIterations);// j
